...
procedure preguntas (var r: REG_user);
  begin
    with r do
      begin
        email:= Concat('Email-',IntToStr(random(1000)+100),'.com');
        writeln ('Email: ',email);
        revista:= Concat('Revista-',IntToStr(random(100) + 50));
        writeln ('Revista: ',revista);
        rol:= random(4) + 1;
        writeln ('Rol: ',rol);
        cantDias:= random(200) + 1;
        writeln ('Cantidad de dias desde el ultimo acceso: ',cantDias);
      end;  
  end;
...
procedure incisos (L: Lista; var ln: Lista; var v: vecIncB);
  ...
  begin
    ...
    while (L <> nil) do
      begin
        // INCISOS A
        if (L^.dato.revista = 'Revista-100') then

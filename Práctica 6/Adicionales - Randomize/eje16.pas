...
procedure preguntas (var r: REG_app);
  begin
    with r do
      begin
        nombre:= Concat('Nombre-',IntToStr(random(5000)));
        apellido:= Concat('Apellido-',IntToStr(random(5000)));
        km:= random(1000) - 10;
        if (km > -1) then
          begin
            mins:= random(100);
            pais:= Concat('Pais-',IntToStr(random(50)));
            ciudadS:= Concat('CiudadSalida-',IntToStr(random(4000)));
            ciudadF:= Concat('CiudadFinalizo-',IntToStr(random(4000)));
          end;
      end;
  end;

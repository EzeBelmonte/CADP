...
procedure preguntas (var r: REG_pelicula);
  begin
    with r do
      begin
        codPeli:= random(500) + 1;
        titulo:= Concat('Tutulo-',IntToStr(random(30000)));
        codGen:=  random(8) + 1;
        puntaje:= random(10) + 1;
      end;
  end;
...
procedure leerCrit (var r: REG_critico);
  begin
    with r do
      begin
        dni:= random(32767);
        apellido:= Concat('Apellido-',IntToStr(dni));
        nombre:= Concat('Nombre-',IntToStr(dni));
        cod:= random(500) - 17;
        puntaje:= random(10) + 1;
      end;
  end;
    ...
    while (r.cod <> -1) do
      begin
        ...
        while (auxL <> nil) and (auxL^.dato.codPeli <> codAux) do
          auxL:= auxL^.sig;
        //auxL^.dato.puntaje:= promedio; //Por usar Randomize, esto no funciona

        //Inciso B
        if (promMax < promedio) then
          begin
            promMax:= promedio;
            //genMax:= auxL^.dato.codGen; //Por usar Randomize, esto no funciona
          end;
      end;

  ...
  begin
    with r do
      begin
        ...
        numAlumno:= random(2000) - 17;
        if (numAlumno <> - 1) then
          begin
            apellido:= Concat('Apellido-',IntToStr(numAlumno));
            nombres:= Concat('Nombre-',IntToStr(numAlumno));
            correo:= Concat('Correo-',IntToStr(numAlumno),'.com');
            anioIngreso:= random(10) + 2000;
            anioEgreso:= random(7) + 2016;

...
procedure preguntas (var r: REG_empresa);
  ...
  begin
    with r do
      begin
        write ('Codigo empresa: '); readln(cod);
        if (cod <> -1) then
          begin
            write ('Nombre de empresa: '); readln(nombre);
            write ('Empresa privada o estatal: '); readln(privEsta);
            write ('Ciudad: '); readln(ciudad);
            writeln ('=== Cultivo ===');
            dimL:= 0;
            write ('Cant hectareas: '); readln(ha);
            while (dimL <= dimF) and (ha <> 0) do
              begin
                dimL:= dimL + 1;
                write ('Tipo: '); readln(cultivo[dimL].tipo);
                write ('Cantidad de meses: '); readln(cultivo[dimL].meses);

...
procedure preguntas (var r: REG_transferencia);
  begin
    with r do
      begin
        write ('Numero cuenta origen: '); readln(numOri);
        write ('D.N.I de cuenta origen: '); readln(dniOri);
        write ('Numero cuenta destino: '); readln(numDes);
        write ('D.N.I de cuenta destino: '); readln(dniDes);
        writeln ('=== Fecha ===');
        ...
        if (fecha.anio = 2018) then
          begin
            writeln ('=== Hora ===');
            write ('Hora: '); readln(hora.hora);
            write ('Minutos: '); readln(hora.min);
            write ('Monto: '); readln(monto);
            write ('Codigo de motivo: '); readln(cod);
          end;
      end;  
  end;
